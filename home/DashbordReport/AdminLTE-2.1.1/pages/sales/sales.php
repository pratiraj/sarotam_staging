                                        ...
                                        <tbody>
                                            ...
                                            if (isset($store_id) && $store_id != "") {
                                                if (isset($date) && $date != "") {
                                                    ...
                                                    $query = "select date(order_date) as date,sum(order_amt) as sale,count(id) as bill from it_orders where store_id = $store_id and date(order_date) between $date_1 and $date_2  group by date(order_date);";
                                                } else {
                                                    $query = "select date(order_date) as date,sum(order_amt) as sale,count(id) as bill from it_orders where store_id = $store_id  group by date(order_date);";
                                                }
                                            } else {
                                                $query = "select date(order_date) as date,sum(order_amt) as sale,count(id) as bill from it_orders where store_id = -1 group by date(order_date);";
                                            }
                                            $obj = $db->fetchObjectArray($query);
                                            $count = 0;
                                            foreach ($obj as $user) {
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?php echo date("D", strtotime($user->date)) ?></td>
                                                    <td><?php echo $user->sale; ?></td>
                                                    <td><?php echo $user->bill; ?></td>
                                                    <td><?php
                                                        $apc = ($user->sale / $user->bill);
                                                        echo round($apc, 2);
                                                        ?></td>

                                                </tr>
                                                <?php
                                                $total_val = $total_val + $user->sale;
                                                $total_bill = $total_bill + $user->bill;
                                                $avg_apc = $avg_apc + $apc;
                                                $count ++;
                                            }
                                            if ($total_bill != 0) {
                                                $total_apc = ($total_val / $total_bill);
                                            }
                                            ?>
                                        </tbody>
                                        <tfoot>
                                            <tr>
                                                <th></th>
                                                <th>Total</th>
                                                <th><?php echo $total_val; ?></th>
                                                <th><?php echo $total_bill; ?></th>
                                                <th><?php echo round($total_apc, 2); ?></th>
                                            </tr>
                                            <?php if ($count != 0) { ?>
                                                <tr>
                                                    <th></th>
                                                    <th>Average</th>
                                                    <th><?php echo round(($total_val / $count), 2); ?></th>
                                                    <th><?php echo round(($total_bill / $count), 2); ?></th>
                                                    <th><?php echo round(($avg_apc / $count), 2); ?></th>
                                                </tr>
                                            <?php } ?>
                                        </tfoot>
                                    </table>
