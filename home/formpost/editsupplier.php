...
try{
    ...
    $suppid = isset($suppid) && trim($suppid) != "" ? intval($suppid) : false;
    if(!$suppid){ $error['missing_suppid'] = "Not able to get Supplier ID for update"; }
    ...
    if(count($error) == 0){
       $supp_id = $dbl->updateSupplier($dateofentry,$kycnumber,$companyname,$bankname,$bankaccno,$bankbranchname,$firmtype,$currency,
