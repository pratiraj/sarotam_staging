...
//    }
    $suppliercode = $dbl->getTransporterCode();
    $suppliercode= sprintf("%05d",$suppliercode->snumber+1);
    $suppliercode_t = $suppliercode;
    ...
    if(count($error) == 0){
       $supp_id = $dbl->insertTransporter($dateofentry,$kycnumber,$companyname,$suppliercode_t,$bankname,$bankaccno,$bankbranchname,$firmtype,$currency,
       ...
       if($supp_id > 0){
           $dbl->updateTransportersCode($firstCharacter,$suppliercode);
       }
    }
} catch (Exception $ex) {
...
if (count($error) > 0) {
    ...
    $redirect = 'transportation/create';
} else {
