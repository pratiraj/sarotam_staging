...
try {
    ...
    $grnid = isset($grnid) && intval($grnid) > 0 ? $grnid : false;
    if (!$grnid) {
        $error["missing_po"] = "Not able to get GRN Id.";
    }
    ...
    $totalQty = 0;
    $totalValue = 0;
    $grnStatus = GRNStatus::Created;
    $grnitemstatus = GRNItemStatus::Submitted;
    $StockDiaryReason = StockDiaryReason::PurchaseIn;
    $obj_grn = $dbl->getGRNDetails($grnid); // dcid
    $obj_grnitems = $dbl->getGRNItems($grnid);
    foreach ($obj_grnitems as $grnitems){
        $totalQty =  $totalQty + $grnitems->qty;
        $totalValue = $totalValue + $grnitems->totalvalue;
    }
//    $ponum = "SA" . $dbl->fetchNextPONumber();
    ...
    if (count($error) == 0) {
        $grn_id = $dbl->saveGRN($grnid,$grnStatus,$totalQty,$totalValue);
        //echo $grn_id;
        $grnitem_id = $dbl->updateGRNItemStatus($grnid,$grnitemstatus,$StockDiaryReason,$obj_grn->dcid);
...
if (count($error) > 0) {
    ...
    $redirect = 'grn/additem/grnid=' . $pid;
} else {
    ...
    $redirect = "grn/grnstatus=".$grnStatus;
