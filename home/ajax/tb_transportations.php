...
$aColumns = array('id','name','email','phoneno','status','action');
$sColumns = array('s.id','s.name','s.email', 's.phoneno','s.inactive');
...
$sQuery = "
	select SQL_CALC_FOUND_ROWS s.id,s.company_name,s.email1,s.phone1,s.inactive
	from it_transporters s
	$sWhere 
	$sOrder
	$sLimit
";
