...
try{
    ...
    $postatus = POStatus::AwaitingCancel;
    ...
    //if ($objpo->po_status == $postatus) {
            if (isset($objDirector->email) && trim($objDirector->email) != "") {
                ...
                foreach ($arr_to as $to) {
                    //echo $to;
                    $subject = "Purchase Order No " . $objpo->pono." is Awaiting For Cancel."; //." Date ". ddmmyy_date($objpo->submitdate);
                    $body = '<p>New Purchasse Order No '.$objpo->pono.' is Awaiting For Cancel. Kindly Login and Cancel This PO.</p>
    ...
    if(count($error) == 0){
        $dbl->cancelAwaitingPO($poid,$postatus,$userid,$remarks);
        $resp = array(
            "error" => "0",
            "msg" => "success"
        );
