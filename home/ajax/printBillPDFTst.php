           ...
           foreach($invitemsobjs as $invitemobj){ 
               if(isset($invitemobj) && !empty($invitemobj) && $invitemobj != null){
                  ...
                  // $total_qty = $total_qty + $invitemobj->qty;
                   ...
                   $roundQty = round($invitemobj->qty,4);
                   ...
                   $taxable_value = $invitemobj->rate * $roundQty;
                   $total_taxable_amt = $total_taxable_amt + $taxable_value;
                   $cgst_value = $taxable_value *   ($invitemobj->cgst_percent / 100);
                   $total_cgst_amt = $total_cgst_amt + $cgst_value;
                   $sgst_value = $taxable_value *   ($invitemobj->sgst_percent / 100);
                   $total_sgst_amt = $total_sgst_amt + $sgst_value;
                   $line_Total = $taxable_value + $cgst_value + $sgst_value;
                   $all_total_val = $all_total_val + $line_Total;
                   //echo $roundLineTotal;
                                                               //echo $itemname;
                    ...
                    $htmlstable .= '<tr>
                        ...
                        ';
                        //$htmlstable .= '<td style="align=left;font-size:8px;width:23%;">'.trim($invitemobj->product).'</td>';
                        ...
                        <td style="align=right;font-size:8px;width:5%;">'.$spdf->Currency(number_format((float)trim($invitemobj->rate), 2, '.', '')).'</td>
                        
                            
                        <td style="align=right;font-size:8px;width:7%;">'.$spdf->Currency(number_format((float)trim($taxable_value), 2, '.', '')).'</td>';
                        if($obj_cr->state == $customer_state_id || $customer_state_id == null){
                            
                            $htmlstable .='<td style="align=center;font-size:8px;width:5%;">'.trim($invitemobj->cgst_percent).'</td>
                                
                            <td style="align=right;font-size:8px;width:8%;">'.$spdf->Currency(number_format((float)$cgst_value, 2, '.', '')).'</td>
                            
                            <td style="align=center;font-size:8px;width:5%;">'.trim($invitemobj->sgst_percent).'</td>
                                
                            <td style="align=right;font-size:8px;width:8%;">'.$spdf->Currency(number_format((float)$sgst_value, 2, '.', '')).'</td>
                            
                            <td style="align=right;font-size:8px;width:7%;">'.$spdf->Currency(number_format((float)$roundLineTotal, 2, '.', '')).'</td>    
                            <td style="align=right;font-size:8px;width:10%;">'.$spdf->Currency(number_format((float)$line_Total, 2, '.', '')).'</td>    
