...
$aColumns = array('name','day_of_week','active','createtime','action');
$sColumns = array('s.name','s.day_of_week','s.active','s.createtime','s.action');
...
$sQuery = "
	select SQL_CALC_FOUND_ROWS s.id,s.name,s.day_of_week,s.is_active,s.createtime
	from it_events s
	$sWhere 
	$sOrder
	$sLimit
...
{       $tot_stk = 0;
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ )
	{
             if ($aColumns[$i] == 'name') {
                 $row[] = $obj->name;
             }else if($aColumns[$i] == 'day_of_week'){
                 $row[] = $obj->day_of_week;
                ...
                if(trim($obj->is_active) == 1)
                 ...
                 $row[] = '<button type="button" onclick="editEvent(' . $obj->id . ')">Edit</button>';            
             }else{
                 $row[] = "-";
             }   
