...
$aColumns = array('transferno','fromloc','toloc','qty','value','createdby','createdon','action','challan');
$sColumns = array('s.transferno', 'd.dc_name', 's.to_location_id', 's.tot_qty', 's.tot_value','u.name','s.transferdate');
...
$sWhere = " where case when s.from_location_type = 1 then s.from_location_id = d.id else s.from_location_id = c.id end and case when s.to_location_type = 1 "
        . "then s.to_location_id = d.id else s.to_location_id = c.id end and u.id = s.createdby and s.status = $status and s.inactive = 0";
//$sQuery = "
...
//                           . '<button class="btn btn-primary" type="button" onclick="deleteGRN(' . $obj->id . ')">Delete</button>';            
                }else if($status == StockTransferStatus::AwaitingIn || $status == StockTransferStatus::Completed){
                   $row[] = '<button class="btn btn-primary" type="button" onclick="pullStockTransfer(' . $obj->id . ')">View</button>

                             <br/><br/><form target="_blank" method="post" action="formpost/stpdf.php">
                                <input type="hidden" name="transferid" id="transferid" value='.$obj->id.'/>
             	...
             	// if($usertype == UserType::PurchaseOfficer && $status == StockTransferStatus::AwaitingIn){
             		
             	// }else if($usertype == UserType::PurchaseOfficer && $status == StockTransferStatus::Completed){
