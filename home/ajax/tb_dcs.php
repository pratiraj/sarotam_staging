...
$sColumns = array('s.id','s.dc_name','st.STATE','s.address','s.contact_person', 's.phoneno', 's.emailaddress', 's.gstno', 's.panno' ,'s.active');
...
$sQuery = "
	select SQL_CALC_FOUND_ROWS s.id,st.STATE as state,s.dc_name as name,s.address,s.contact_person,s.phoneno,s.emailaddress,
        s.gstno,s.panno,s.inactive
	from it_dc_master s, states st
	$sWhere 
	$sOrder
	$sLimit
";
...
//$aColumns = array('id','name','state','address','graddress','contact_person','phone','email','gstno','panno','status','action');        
	for ( $i=0 ; $i<count($aColumns) ; $i++ ){
             if ($aColumns[$i] == 'id') {
                 ...
                 $row[] = '<button type="button" onclick="editDC(' . $obj->id . ')">Edit</button>';            
             }else{
                 $row[] = "-";
             }   
