...
try{
    ...
    //if ($objpo->po_status == $postatus) {
            if (isset($objDirector->email) && trim($objDirector->email) != "") {
                ...
                foreach ($arr_to as $to) {
                    //echo $to;
                    $subject = "Purchase Order No " . $objpo->pono." is Cancelled."; //." Date ". ddmmyy_date($objpo->submitdate);
                    $body = '<p>New Purchasse Order No '.$objpo->pono.' is Cancelled.</p>
    ...
    if(count($error) == 0){
        $dbl->approvecancelPO($poid,$postatus,$userid,$remarks);
        $resp = array(
            "error" => "0",
            "msg" => "success"
        );
