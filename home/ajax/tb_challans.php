...
$sQuery = "
	select SQL_CALC_FOUND_ROWS stc.id, stc.st_id ,stc.challan_no,s.from_location_type, case when s.from_location_type = 1 then d.dc_name else c.crcode end as fromloc,
        case when s.from_location_type = 1 then upper(c.crcode) else d.dc_name end as toloc,stc.total_qty,stc.total_value,u.name,stc.submittedDate,s.createtime from it_stock_transfer s,
